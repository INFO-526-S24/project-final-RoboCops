...
The first animated plot provides a detailed depiction of state hotspots for the years 2015, 2016, and 2017. These hotspots are identified through meticulous consideration of population data and the application of a critical metric known as "shooting intensity." This intensity is computed as fatalities per million people, ensuring a standardized assessment of states irrespective of their population size. To ensure clarity and inclusivity, both U.S. and world maps are employed in the visualization process. Given that the U.S. map typically excludes Alaska, a comprehensive approach involves manually incorporating Alaska from the `world map` and custom annotating it onto the `U.S. map` to present a holistic view. States where the shooting intensity surpasses the threshold of 3 are specifically highlighted using `geom_label_repel`, drawing attention to regions experiencing heightened incidents. Furthermore, the plot includes annotations identifying the top three hotspot states, ranked by shooting intensity and total fatalities. This multifaceted approach offers a nuanced understanding of incident frequency and impact, facilitating comprehensive analysis across diverse geographical areas.

The second animated plot offers an intricate portrayal of major city hotspots for the years 2015, 2016, and 2017. These hotspots are identified using the same metric of "shooting intensity" as in the previous plot. This intensity is calculated as fatalities per million people, ensuring a consistent evaluation of cities regardless of their population size. To maintain coherence and comprehensiveness, the visualization of states from the previous plot is retained to provide contextual continuity. Moreover, cities are marked using `geom_point`, and those with a high shooting intensity exceeding 28 are further highlighted using `geom_label_repel`. This approach effectively showcases cities within states with moderate shooting intensity that exhibit significantly higher shooting intensity, thus emerging as notable hotspots. This layered depiction elucidates the presence of cities with heightened incidents within states with relatively lower overall intensity, contributing to a nuanced understanding of localized dynamics within broader geographical contexts.
...
# Check the updated state column
...
animated_heatmap <- ggplot(long_data, aes(x = racial_group, y = state, fill = intensity)) +
  geom_tile() +
  scale_fill_viridis_c(option = "plasma") +
  labs(title = "State vs. Racial Intensity Heatmap: Year {frame_time}", x = "Racial Group", y = "State") +
  theme_minimal() +
